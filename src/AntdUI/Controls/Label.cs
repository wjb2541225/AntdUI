...
namespace AntdUI
{
    ...
    public class Label : IControl, ShadowConfig, IEventListener
    {
        ...
        public Image? PrefixImage
        {
            ...
            set
            {
                if (_prefixImage != value)
                {
                    _prefixImage = value;
                    IOnSizeChanged();
                    if (BeforeAutoSize()) Invalidate();
                    OnPropertyChanged(nameof(PrefixImage));
                }
            }
        }
        ...
        void PaintText(Canvas g, string? text, Color color, Rectangle rect_read)
        {
            if (!string.IsNullOrEmpty(text))
            {
                ...
                if (has_prefixText || has_suffixText || has_prefix || has_suffix || _prefixImage != null || _suffixImage != null)
                {
                    switch (textAlign)
                    {
